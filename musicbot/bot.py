...
#######################################################################################################################
    ...
    async def cmd_help(self, message, channel, command=None):
        ...
        desc = '```\n' + ', '.join(self.commands) + '\n```\n' + self.str.get(
            'cmd-help-response', 'For information about a particular command, run `{}help [command]`\n'
                                 'For further help, see https://LouisFrancois-Bots.github.io/MusicBot/').format(prefix)
        if not self.is_all:
    ...
    async def cmd_karaoke(self, player, channel, author):
        ...
        return Response("\N{OK HAND SIGN} โหมด Karaoke " + ['ปิดแล้ว', 'เปิดแล้ว'][player.karaoke_mode], delete_after=15)
    ...
    async def cmd_np(self, player, channel, guild, message):
        ...
        if player.current_entry:
            ...
            if player.current_entry.meta.get('channel', False) and player.current_entry.meta.get('author', False):
                np_text = self.str.get('cmd-np-reply-author', "Now {action}: **{title}** เพิ่มโดย**{author}**\nProgress: {progress_bar} {progress}\n\N{WHITE RIGHT POINTING BACKHAND INDEX} <{url}>").format(
                    action=action_text,
    ...
    async def cmd_remove(self, user_mentions, message, author, permissions, channel, player, index=None):
                ...
                if permissions.remove or author == user:
                    try:
                        ...
                        if len(entry_indexes) > 1:
                            entry_text += 's'
                        return Response(self.str.get('cmd-remove-reply', "Removed `{0}` เพิ่มโดย`{1}`").format(entry_text, user.name).strip())
        ...
        if permissions.remove or author == player.playlist.get_entry_at_index(index - 1).meta.get('author', None):
            ...
            if entry.meta.get('channel', False) and entry.meta.get('author', False):
                return Response(self.str.get('cmd-remove-reply-author', "Removed entry `{0}` เพิ่มโดย`{1}`").format(entry.title, entry.meta['author'].name).strip())
            else:
    ...
    async def cmd_queue(self, channel, player):
        ...
        if player.is_playing:
            ...
            if player.current_entry.meta.get('channel', False) and player.current_entry.meta.get('author', False):
                lines.append(self.str.get('cmd-queue-playing-author', "Currently playing: `{0}` เพิ่มโดย`{1}` {2}\n").format(
    ...
    async def cmd_clean(self, message, channel, guild, author, search_range=50):
        ...
        if self.user.bot:
            if channel.permissions_for(guild.me).manage_messages:
                ...
                return Response(self.str.get('cmd-clean-reply', 'ทำความสะอาดไป {0} ข้อความ{1}.').format(len(deleted), 's' * bool(deleted)), delete_after=15)
